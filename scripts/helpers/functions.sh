...
dev_create_containers() {
    ...
    log_print "Building and starting custom Apache PHP server for OSU website development...";
    docker build "${APACHE_DOCKERFILE_FOLDER}" -t "${APACHE_PHP_IMAGE}"
    docker run -d \
    ...
    simple_print "";
    log_print "Local OSU website development environment setup complete.";
    log_print "3 docker containers were started:";
    log_print "    - ${MYSQL_CONTAINER_NAME}";
    log_print "    - ${PHP_MY_ADMIN_CONTAINER_NAME}";
    log_print "    - ${APACHE_PHP_CONTAINER_NAME}";
    log_print "";
    log_print "All containers are part of the ${NETWORK_NAME} docker bridge network.";
    log_print "";
    log_print "The script has created a MySQL database server with the following credentials:";
    log_print "    host: ${MYSQL_CONTAINER_NAME}";
    log_print "    user: root";
    log_print "    pass: ${DB_PASSWORD}";
    log_print "    name: ${DB_NAME}";
    log_print;
    log_print "phpMyAdmin is available at http://localhost:${PHP_MY_ADMIN_LOCAL_PORT}";
    log_print "";
    log_print "The Apache PHP server serving content for the website is listening at";
    log_print "http://localhost:${APACHE_PHP_LOCAL_PORT}";
...
dev_setup_containers() {
    ...
    docker exec -it "${APACHE_PHP_CONTAINER_NAME}" /bin/bash -c "cd /var/www/html/scripts; php create-user.php 'admin' 'admin_first' 'admin_last' 3"
    docker exec -it "${APACHE_PHP_CONTAINER_NAME}" /bin/bash -c "cd /var/www/html/scripts; php create-user.php 'student' 'student_first' 'student_last' 1"
    docker exec -it "${APACHE_PHP_CONTAINER_NAME}" /bin/bash -c "cd /var/www/html/scripts; php create-user.php 'proposer' 'proposer_first' 'proposer_last' 2"
